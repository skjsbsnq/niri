...
`arch-update.sh` also maintains a patched `xwayland-satellite` under the legacy path `~/.local/lib/niri/xwayland-satellite-minimize` by default. This is the X11 fallback needed by apps such as Linux WeChat whose visible window still goes through xwayland-satellite. The combined patch bridges EWMH maximize requests to `xdg_toplevel.set_maximized()` / `unset_maximized()`, forwards `WM_CHANGE_STATE/IconicState` minimize requests, and refreshes stale Wayland clipboard offers so X11 apps can see clipboard updates from Wayland apps. See `docs/old/xwayland-satellite-clipboard.md` before changing the patch or clipboard bridge behavior.

The Tahoe session defaults `ELECTRON_OZONE_PLATFORM_HINT=wayland`. This is intentional for Linux QQ and other Electron clients: the configured `eDP-2` output uses scale `1.25`, so an X11 buffer must pass through Xwayland fractional viewport scaling and linear sampling and can look slightly soft. Use X11 only as a per-app compatibility fallback. A user-local `.desktop` file or `~/.config/qq-flags.conf` containing `--ozone-platform=x11` overrides the session default and must be changed or removed before QQ can use native Wayland.
