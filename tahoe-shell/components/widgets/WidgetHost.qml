...
//
// 网格：4×4 单元格（屏宽 4 列、屏高方向最多 4 行），每屏一份宿主，
// 配置按屏持久化（widgets.json 以屏幕名为根，数组格式 [{id,size,col,row}]）。
//
// region 计数保护（P-5）：每个小部件 1 个 region；每屏网格容量 ≤16，
// 单屏不可能超 32，但配置文件跨屏总条目可能超限（最多 16×N），故
// 加载时对【每屏条目上限 16】截断，并把被截断的条目数计入超限横幅
// （可见反馈，不静默失效）。
PanelWindow {
    id: root
    ...
    readonly property real cellSize: Math.max(1, screenWidth / Grid.GRID_COLS)
    ...
    function createWidgetInstance(entry) {
        ...
        var obj = component.createObject(widgetLayer, {
            "widgetSize": String(entry.size || "small"),
    ...
    function loadConfig() {
        ...
        // 未显示数 = 本屏剔除 + 跨屏超额（P-5 可见反馈的真实数量）。
        root.overflowCount = state.removed.length + Math.max(0, root.totalEntryCount() - Grid.LIMIT_ITEMS);
        root.rebuildWidgets();
    ...
    function totalEntryCount() {
        ...
        if (Array.isArray(parsed)) {
            ...
            var sKeys = Object.keys(parsed);
            for (var i = 0; i < sKeys.length; i++) {
                if (Array.isArray(parsed[sKeys[i]]))
                    total += parsed[sKeys[i]].length;
            }
        }
        return total;
    }
    ...
    Item {
        ...
        Rectangle {
            ...
            Text {
                ...
                text: "小部件配置超限：" + root.overflowCount + " 个未显示（上限 "
                    + Grid.LIMIT_ITEMS + "）"
                color: "#ffffff"
                font.pixelSize: 13
