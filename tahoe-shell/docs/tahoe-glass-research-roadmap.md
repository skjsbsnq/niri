...
本文记录对 Tahoe 玻璃效果链路的只读研究结论，并给出后续改进路线。研究期间未修改实现代码，未运行脚本，未读取 baseline 文件。本文本身是为了保存结论而新增。
...
- QML 仍然在玻璃 item 上绘制 tint、border、inner stroke、hairline，这些覆盖层会影响最终玻璃观感。
...
后续如果 niri material 拥有足够高光、内阴影和折射，QML 覆盖层应逐步降权。
...
3. 然后逐步降低 QML 静态 stroke/hairline 的存在感。
