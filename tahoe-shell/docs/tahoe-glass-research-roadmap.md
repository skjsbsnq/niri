...
### Phase 4：引入更接近 AndroidLiquidGlass 的 shader 表现（已完成）

目标：把 niri Tahoe material 从普通 blur/shadow 扩展为真正的 liquid glass。

状态：已完成（2026-06-17）。落地细节见 `tahoe-shell/docs/phase4-liquid-glass.md` 的 2026-06-17 章节。要点：

- 新增 `rounded_rect_sdf.frag`：标准 iquilezles 圆角矩形 SDF（`niri_sd_rounded_rect` + 外向梯度 `niri_sd_rounded_rect_grad`），per-corner 选择与 `rounding_alpha.frag` 一致。这让边缘高光、折射偏移、内阴影按圆角形状而不是轴对齐包围矩形计算——是修复圆角处错位的核心。
- `postprocess.frag` 新增两个材质 uniform：`inner_shadow`（SDF 法线点乘右下光向的内边缘暗化带，shader 内实现，不走 knockout+blur）、`lens_depth`（中心径向凸起透镜，circular-ease，大表面衰减）。
- `clipped_surface.frag` 新增 chromatic aberration：`chromatic > 0` 时把背景采样拆成沿折射方向的 R/G/B 三采样（+2 次纹理采样，默认 0 不增成本）。
- 配置新增 per-material 旋钮 `inner-shadow`/`chromatic`/`lens-depth`（`BackgroundEffect{,Rule}` + `TahoeGlassMaterialRule`），`GlassOptions` 与 `framebuffer_effect.rs`/`xray.rs` 的 uniform 数组从 `[11]` 扩到 `[14]`。
- 协议 v1 → v2：`set_region` 新增 `interaction`（fixed，0..1）。niri 在 `make_region` clamp 到 [0,1]，在 `render_region` 按 `(1 + interaction)` 缩放 `edge_highlight`/`refraction`/`inner_shadow`/`chromatic`/`lens_depth`（compositor-internal material easing，不动 shader/region 几何）。quickshell fork 重新生成协议，`TahoeGlassRegion` 暴露 `interaction` QML 属性。
- QML 示范：`Dock.qml` 绑 `dockHovered`（hover 增强），`NotificationToast.qml` 绑 `card.opacity`（随 toast 进出 ease）。其余 region 保持 `interaction: 0`。
- `tahoe-phase0.kdl` 给 6 个 material 设了保守起点值（backdrop 保持 0，全屏不应扭曲）；snap preview 加了 alpha-scaled 的 `inner-shadow`/`lens-depth`。
- 保留约束：region 几何仍用 bounded `NumberAnimation`；新效果都被 `glass_surface_detail()` 在大表面衰减；VMware/software GPU 先验证再提数值。
...
- region 支持 `intensity`、`interaction` 或 `state` 参数。→ 实现为 `interaction`。
- QML 可为 Dock hover、toast enter、popup open 传递轻量交互值。→ Dock hover、toast enter 已接。
- 如果暂不扩协议，也可以 compositor 内部根据 region 生命周期做 open/close material easing。→ 本实现同时选了"扩协议 + compositor 内部 easing"：协议传值，easing 在 compositor 内做。

### Phase 5：动画模型重构（已完成）

目标：动画更活，但不牺牲稳定性。

状态：已完成（2026-06-17）。本阶段把交互动画从 glass region 几何迁移到内容层与 compositor material easing：

- Tahoe 私有协议 `tahoe_glass_surface_v1.set_region` 扩展 `material_alpha`（0..1），Quickshell `TahoeGlassRegion.materialAlpha` 暴露给 QML；niri clamp 后用于淡入/淡出 tint、edge highlight、refraction、inner shadow、chromatic、lens depth 和 shadow alpha。
- popup open/close（ControlCenter、MenuPopup、BatteryPopup、NotificationCenter、TrayMenu）不再移动主 glass region 的 `y`。Region 使用固定 `x/y/width/height`，视觉打开关闭由内容层 `opacity` + `Scale` transform 和 `materialAlpha` 完成。
- Dock hover 保持 icon scale/lift 在内容层，glass region 继续用 `interaction` 提高 compositor highlight/refraction，并用 `materialAlpha` 随 Dock fade 退出。
- Toast enter/exit 保留 `card.x` 的 bounded `NumberAnimation`，同时用 `materialAlpha: card.opacity` 让 compositor 材质随 toast 淡入淡出。
- Launchpad backdrop 使用 `interaction/materialAlpha: backdrop.opacity` 驱动 fullscreen material 过渡，并降低 QML `FillBackdrop` 静态色层权重，让 blur/refraction/tint 主要由 compositor material 承担。
- Spotlight 的两块 region 改为显式稳定坐标，避免父级 content scale 影响 region bounds；两块 region 独立随各自 opacity 驱动 material。
- Guardrail 增加 `materialAlpha` 检查，并检查主要 popup 不再对 glass region `y` 做 open/close 动画。
