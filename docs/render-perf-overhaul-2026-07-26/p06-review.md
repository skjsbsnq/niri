...
> 指针状态：本 commit 当时未前移子模块指针（主检出树由 P05 会话持有未提交改动）；
> 已随 P05 落地（外层 dd92711）统一前移——外层现记录 niri @ 9ce7a720，其祖先链
> 包含 78540ae2。**部署已落地（2026-07-26 20:52 会话重启）**：运行中的
> `~/.local/bin/niri` 即 9ce7a720，本机制默认启用（未设 kill-switch env）；A/B
> 实测仍按下文留待 P08。
