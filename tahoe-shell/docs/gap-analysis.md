...
当前 `tahoe-shell/components/TopBar.qml` 状态区已有：系统托盘 `Tray`、workspace 数字按钮、通知铃铛、Spotlight 入口、时钟和控制中心入口。**仍没有**：电池、亮度、音量、输入法、控制中心图标组（Wi-Fi/BT 那组小图标）。
...
> 当前项目状态：**niri fork 那一层（minimize/snap/glass shader/snap preview）是真东西且完成度不错；Quickshell shell 那一层原先是"macOS 的皮 + 没有功能的骨头"，现已完成六项改造（控制中心 + 真通知系统 + 动画 spring 化 + 玻璃参数重做 + Spotlight/Launchpad 搜索 + snap preview 重做），但菜单仍是假的，Spotlight 仍只是应用搜索，不是完整全局搜索**。Web 参考项目是一份现成的"功能清单 + 视觉参数表 + 动画曲线表"，应该继续当成蓝图逐项翻译成 QML。下一项：服务层接 niri IPC socket / 真机验收。
