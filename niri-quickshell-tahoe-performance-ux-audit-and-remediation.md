...
- 多个玻璃 surface 不能共享廉价 backdrop 结果。

修复方向：

1. 建立按材质和区域选择的采样策略。
2. 静态或大面积面板优先共享 xray/backdrop。
3. 只有确实需要看到实时窗口内容的局部区域才走 nonxray。
4. 不新增第二套玻璃组件或第二个 blur API。
...
3. niri 根据 material 和 surface 类型选择 xray/nonxray。
4. blur 资源复用。
5. damage 精确覆盖变化区域。
...
- 减少默认 nonxray framebuffer capture。
- 保留真实玻璃效果需要的局部实时采样。
...
| R04 玻璃采样策略 | verified | 沿用唯一 Tahoe material/BackgroundEffect `xray` 入口：`panel`/`launcher`/`dock`/`backdrop` 改用共享 backdrop，`pill`/`menu`/`toast` 与普通窗口保留局部实时 framebuffer；TopBar/Dock 绑定和 fallback 同策略由治理测试锁定，配置已热加载并部署，截图确认 TopBar/动态岛无视觉破坏；同一旧会话 10 秒 `nvidia-smi pmon` 中 niri 有效 SM 样本均值由约 21.1% 降至 11.4%（约 46%，动态壁纸和前台负载仍为干扰项）；目标测试 4/4、Shell 709/709、niri 单线程 297/297、niri-config 35/35（wiki 1/1）、niri-ipc 3/3（doctest 1/1）、Quickshell Tahoe CTest 12/12、release check、配置 validate、guardrails 与 diff check 通过；默认并行仅复现既有 4 个 layer-close 时序失败，全仓 fmt 与未安装 Clippy 仍为既有 R15 门禁 |
| R05 Blur/FBO 缓存 | pending |  |
| R06 Shader 优化 | pending |  |
| R07 Damage 与动态岛 commit | pending |  |
| R08 缩略图异步与原子发布 | pending |  |
| R09 Layer close 与 spring | pending |  |
| R10 动画所有权 | pending |  |
| R11 设置高频写入 | pending |  |
| R12 服务、搜索和轮询 | pending |  |
| R13 通知、锁屏与启动错误 | pending |  |
