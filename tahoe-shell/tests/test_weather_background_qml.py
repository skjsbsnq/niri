#!/usr/bin/env python3
"""Load production WeatherBackground via qmltestrunner."""
...
QML_TEST = Path(__file__).with_name("tst_weather_background.qml")


def test_weather_background_loads_in_qmltestrunner() -> None:
    qt6_runner = Path("/usr/lib/qt6/bin/qmltestrunner")
    runner = str(qt6_runner) if qt6_runner.is_file() else shutil.which("qmltestrunner")
    assert runner is not None, "Qt 6 qmltestrunner is required for WeatherBackground load coverage"
    env = os.environ.copy()
