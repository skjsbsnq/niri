...
**剩余未做**（留后续）：AirDrop / Stage Manager / 屏幕镜像圆钮（无后端，暂为禁用占位）、深色模式按钮（接 gsettings 是独立任务）。电池电量显示已在 2026-06-15 由顶栏 + `BatteryPopup.qml` 补齐；Wi-Fi / 蓝牙 / 飞行模式点击区域也已修复，飞行模式现在会本地关闭并恢复 Wi-Fi / 蓝牙状态。

下面其余条目（2–8）已经不再全部是占位符；菜单、顶栏和通知中心已有部分闭环，剩余缺口按条目继续收敛：

### 2. 菜单栏左侧 "Tahoe" 下拉 `MenuPopup.qml` —— 部分完成

Web 参考（`index.html` 第 38–71 行）有完整的 Apple 菜单 + 应用菜单 + Finder/File/Edit/Go/Help 多级下拉，每个 `<li><button>` 都有内容。

当前 `tahoe-shell/components/MenuPopup.qml` 已不再只是关闭弹窗：Lock、Sleep、Logout、Restart、Shutdown 已接 `services/Power.qml`，危险动作有确认态。**仍未完成**：About / Settings 还没有真实页面，应用菜单没有接入 DBus appmenu / dbus-menu（roadmap 列为 Phase 5 不优先）。
...
**剩余未做**（留后续）：按 appId 分组堆叠。2026-06-15 已补 `historyModel`、`NotificationCenter.qml`、Clear、单条移除和 Do Not Disturb；DND 下通知写入历史但不弹 toast。
...
当前 `tahoe-shell/components/TopBar.qml` 状态区已有：系统托盘 `Tray`、workspace 数字按钮、通知中心入口、电池入口、Spotlight 入口、时钟和控制中心入口。2026-06-15 已补 UPower 电池 popup，并把系统托盘从薄 Repeater 明确成 SNI 状态区：`IconImage` 渲染 `StatusNotifierItem.icon`、空图标字母 fallback、attention 红色描边；右键菜单也已从原生白菜单改为 Tahoe QML 菜单，能吃到 niri blur。FClash SNI 图标已验证可见；Steam 等 legacy XEmbed 托盘应用如果不发布 SNI，需要 `xembedsniproxy` 之类的桥接服务。**仍没有**：输入法、控制中心图标组（Wi-Fi/BT 那组小图标）、夜间模式状态入口。

### 5. Launchpad —— ✅ 已补搜索框（2026-06-15），仍缺分页
...
- ✅ 有 app grid，✅ 打开 scale 1.1→1 + opacity。
- ✅ 已新增顶部圆角搜索框，复用 `services/Apps.qml` 的 `filteredLaunchpadApps()` 实时过滤 `.desktop` 应用，并支持 Enter 启动第一条结果。
- ❌ 仍没有分页、文件夹。

### 6. Spotlight —— ✅ 基础搜索已完成（2026-06-15）
...
当前 `tahoe-shell/components/Spotlight.qml` 已新增：顶层 overlay、圆角搜索框、右侧 4 个 Tahoe 圆钮、应用搜索结果列表、Enter 启动第一条结果、Escape/外部点击关闭；`TopBar.qml` 已新增 Spotlight 入口。仍未做文件、设置项、计算、命令等完整全局搜索。
...
| Snap preview | `style.css` 第 2455–2469 行：`backdrop-filter: blur(12px)` 半透明圆角矩形 + `transition: opacity 0.3s, left 0.3s, width 0.3s` | niri `render_snap_preview_for_output`：`FramebufferEffectElement` 真实背景 blur/glass + `BorderRenderElement` 圆角填充/描边 + 顶部高光 | 已从直角实心蓝块改为圆角毛玻璃预览；blur 本身暂不随 alpha 混合，tint/高光/描边随 fade alpha 变化 |
...
- ✅ **snap preview 已重做为圆角毛玻璃预览**：复用 framebuffer blur/glass 和 rounded border shader，替换原 `SolidColorRenderElement` 实心块。
...
```text
✅ 控制中心已有真实音量/亮度/Wi-Fi/蓝牙/飞行模式/MPRIS；❌ 键盘亮度/夜间模式仍未做
✅ 真通知系统（2026-06-14 已接 Quickshell NotificationServer，2026-06-15 已补历史中心和 DND，见 §1.3）
✅ 电源/注销/重启/关机/睡眠菜单（2026-06-15 已接 `Power.qml`，危险动作确认）
✅ SNI 系统托盘基础闭环（图标、fallback、attention、自绘右键菜单）；❌ legacy XEmbed 托盘仍需桥接服务
❌ 无锁屏 UI（Super+Alt+L 调 swaylock，不是自己的）
❌ 无截屏标注/录屏/Quick Look (空格预览)
❌ 无输入法指示/切换
✅ 基础 Spotlight / Launchpad 搜索已补；❌ 文件、设置项、计算等完整全局搜索未做
...
5. ~~**snap preview 重做**~~ ✅ 已完成（2026-06-15）。改成 framebuffer blur/glass + 圆角填充/描边 + 顶部高光，不再是实心蓝色块。
6. ~~**加 Spotlight + Launchpad 搜索框**~~ ✅ 已完成（2026-06-15）。Launchpad 顶部搜索框过滤 `.desktop` 应用；Spotlight 新增顶层搜索 overlay、结果列表、顶栏入口和 4 个圆钮。
...
> 当前项目状态：**niri fork 那一层（minimize/snap/glass shader/snap preview）是真东西且完成度不错；Quickshell shell 那一层原先是"macOS 的皮 + 没有功能的骨头"，现已完成控制中心、真通知系统、通知历史/DND、电源菜单、电池状态、SNI 托盘状态区和 Tahoe 右键菜单、动画 spring 化、玻璃参数重做、Spotlight/Launchpad 搜索、snap preview 重做。菜单不再完全是假的，但 About/Settings、app menu/dbus-menu 仍未接；Spotlight 仍只是应用搜索，不是完整全局搜索**。Web 参考项目是一份现成的"功能清单 + 视觉参数表 + 动画曲线表"，应该继续当成蓝图逐项翻译成 QML。下一项：输入法状态 / Dock 右段 / Search provider。
