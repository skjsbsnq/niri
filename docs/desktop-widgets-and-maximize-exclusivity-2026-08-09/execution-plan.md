...
| 2 | B1 建立缺陷复现测试基线 | Rust | 2 | 否 | 完成（be621b18） |
| 3 | B2 修复独占解除时机与粒度 | Rust | 3 | 是 | 完成（f61ebda6） |
| 4 | B3 修复恢复路径可见性抑制 | Rust | 3 | 是 | 完成（38ee5d30） |
| 5 | A1 面板按需加载 | QML | 2 | 是 | 完成（ac347c5） |
| 6 | A2 小部件基类与宿主层 | QML | 2 | 是 | 完成（a0c1e17 + 1a053c5 部署修复） |
| 7 | A3 首批小部件 | QML | 2 | 是 | 完成（164f8ae） |
| 8 | A4 侧栏第三 tab（小部件库） | QML | 2 | 是 | 完成（e26d7d6） |
| 9 | A5 库实时预览 | QML | 2 | 是 | 完成（a0c9d8b） |
