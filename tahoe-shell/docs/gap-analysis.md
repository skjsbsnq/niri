...
### 3. 通知中心 `NotificationToast.qml` —— 单条假通知

当前 `tahoe-shell/components/NotificationToast.qml` 是写死的 "Tahoe / Session ready"，`shell.qml` 第 27–32 行的 Timer 启动 900ms 后强制弹出，3.6 秒自动消失。**不是真通知系统**，没接 notification daemon。
...
❌ 无真通知系统（建议接 Quickshell NotificationServer 或 swaync）
❌ 无电源/注销/重启/关机菜单
