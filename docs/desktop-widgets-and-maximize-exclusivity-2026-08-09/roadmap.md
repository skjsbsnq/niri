...
| A2 | 基类与宿主层 + 电池小部件 | 承载 |
| A3 | 天气 / 日历 / 系统监控 | 内容 |
| A4 | 侧栏第三 tab（小部件库） | **添加入口** |
| A5 | 库实时预览 | **添加体验完整化** |
| A6 | 长按编辑 + 拖动移位 | 编辑 |
| A7 | 边缘 resize 三档切换 | 编辑 |
| A8 | 小部件外观 macOS 1:1 高仿全面重做 | 外观专项 |

**为何添加（A4/A5）先于编辑（A6/A7）**：库 tab 是用户添加小部件的唯一入口。
在 A4 完成前，小部件只能靠手工编辑 `widgets.json` 放置，用户无从添加 ——
此时先做编辑模式是本末倒置。且只有用户能自由添加后（≥3 个小部件），
拖动重排才可被有意义地测试。
...
**为何紧接 A3**：这是用户添加小部件的**唯一入口**。
在此之前（A2/A3）小部件只能靠手工编辑配置文件放置，用户无从添加。
「先能添加，才谈得上编辑」——故库 tab 必须先于编辑模式（A6/A7）。
...
**不做**：带实时预览的画廊（A5）。本任务列表为文字/图标条目即可。
...
**目标**：把 A4 的文字列表升级为带真实预览的画廊。

**为何紧接 A4**：预览是库 tab 的完整形态，与 A4 同属「添加」链路，
应在进入编辑功能之前把添加体验做完整。
此时已有 A2/A3 的四个小部件，画廊有足够内容可展示。
...
**目标**：长按桌面小部件进入编辑模式，可拖动改变位置并持久化。

**为何在 A5 之后**：编辑的前提是桌面上已有用户自己添加的小部件。
A4/A5 完成后用户才能通过界面添加任意数量的小部件，
此时拖动重排才可被有意义地测试（≥3 个小部件）。
...
- 退出编辑模式后抖动动画停止（无常驻动画）
- 通过 A4 库 tab 新添加的小部件同样可被拖动（与配置文件预置的行为一致）
- `pytest` 全绿

---

## A7 — 编辑模式边缘 resize（三档切换）

**问题引用**：`research-report.md` A-0 第 4 条、A-6

**目标**：编辑模式下按住小部件边缘拖动，在 small/medium/large 三档间切换。

**为何最后做**：resize 依赖 A6 建立的编辑模式与网格占用逻辑，
是编辑能力的最后一块。
...
- 持久化正确；重启后尺寸一致
- A4 库 tab 中标注的可选尺寸与实际可切换档位一致
- `pytest` 全绿

---

## A8 — 小部件外观全面重做：macOS 1:1 高仿

**问题引用**：用户 2026-08-12 新需求（A1–A7 完成后提出）：
「对目前的小组件风格很不满意 要求全面重做 效果和外观要和 mac os 1:1 高仿」。

**目标**：把 A2–A7 建成的桌面小部件从「深色不透明平板」全面重做为
macOS Sonoma/Sequoia 桌面小部件的 1:1 高仿外观：自适应磨砂玻璃卡片、
大曲率圆角、SF 级字阶与留白、逐小部件复刻 macOS 原生布局。
功能（网格 / 三档规格 / 添加 / 预览 / 编辑 / 拖动 / resize / 持久化）
一律不变，只重做视觉层。

**为何在 A7 之后**：A1–A7 已交付完整小部件系统，本次是用户对既有
交付物的外观专项重做，不改动任何交互链路。

**范围**：

1. **材质令牌（TahoeGlass.js）**：
...
2. **自适应外观（Widget.qml / WidgetHost / shell.qml）**：
   - `Widget.qml` 增加 `darkMode` 属性（宿主以 Qt.binding 注入），
     玻璃填充/描边/文字/图标颜色按深浅外观切换（深 → 白系，
     浅 → `#1d1d1f` 系）。
   - `WidgetHost` 增加 `darkMode` 注入点；`shell.qml` 接 `shell.darkMode`。
   - `WidgetPreview`（库预览）固定 `darkMode: true`：预览井保持深色，
     白字内容可读（既有 A5 语义）。

3. **逐小部件 macOS 复刻**：
   - 电池（small）：居中电池图标 + 大百分比（Semibold）+ 状态说明；
     充电绿 / 低电量红 / 常规自适应。
   - 天气（medium）：左上城市名 + 大温度（Light）+ 天气状况 +
     今日高低温 + 右侧 44px 天气图标 + 底部逐时条（时间/图标/温度）。
   - 日历（medium）：月份标题 + 今日红字 + 周首字母 + 42 格月历
     （今日红圆白字）。
   - 系统监控（small）：CPU/内存/磁盘细条（macOS 系统色）+ 底部网络行。
   - 字阶：标题 13px、大数值 34px（随尺寸缩放）、说明 12–13px、
     逐时 11–12px，用字重与行距模仿 SF Pro；字体沿用系统 sans
     （Noto Sans CJK SC），**不做**字体打包。

4. **编辑态 chrome（A6/A7 功能不变，只改外观）**：
   - 删除按钮改 macOS 圆形徽标（深灰底白字，z:20 不变，仍右上角）。
   - 「完成」按钮改 macOS 胶囊（深/浅自适应）。
   - 反馈横幅保持功能与位置。

5. **库预览井（LeftSidebarWidgetLibrary.qml）**：预览井圆角改新卡片
   曲率（`widgetRadius`），使预览与实际桌面卡片一致。

6. **视觉契约测试更新**（随本任务一起改）：
   - `test_widget_dock_visual_contract.py` → 改断言自适应玻璃令牌与
     圆角函数（不再断言旧 `FillWidget` / `StrokeWidget` 常量）。
   - `test_battery_widget_layout.py` / `test_weather_widget_layout.py`
     → 更新承重值为新 macOS 布局，几何不等式不变式继续成立。
   - `test_widget_preview.py` → 预览井圆角断言改 `widgetRadius`。

**约束重点**：`constraints.md` P-1（region 几何禁弹簧）、P-3（Motion.js
令牌）、A-C3（禁自建轮询）、A-C4（禁 layer.enabled）、G-6（删除旧令牌）、
G-7（交互/持久化/网格行为不变）、G-8（不加新小部件、不加主题切换设置）。

**不做**：改网格规格 / 尺寸档位 / 配置格式 / 交互手势 / 持久化；
不加新小部件；不改侧栏「系统」「天气」两 tab；不改 niri；
不打包新字体。

**完成判据**：
- 四个小部件在深/浅外观下均为 macOS 风格玻璃卡片，文字对比度正确
  （人工验证：切换外观后小部件立即跟随，无需重启）
- 编辑模式删除徽标与「完成」胶囊风格正确
- 库预览井圆角与桌面卡片曲率一致
- `qmllint --bare` 无新增告警；`pytest` 全绿（含更新后的视觉契约测试）
- grep 断言：`components/widgets/` 无 `SpringAnimation`、
  无硬编码 `duration:` 字面量（P-1 / P-3）
- 回归：A2–A7 全部结构测试绿（拖动 / resize / 添加 / 删除 / 持久化 /
  mask / 预览行为不变）
...
9. 小部件外观为 macOS 1:1 高仿（A8 完成判据全过）
