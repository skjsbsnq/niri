...
| R14 Direct scanout 与背景预算 | in_progress | fullscreen 状态唯一来自 Quickshell foreign-toplevel 已提交的 `fullscreen + screens`，不再用窗口尺寸猜测；TopBar、Dock、Dynamic Island 按输出真实 unmap，任意输出进入 fullscreen 时同时关闭已打开的顶栏弹窗、dismiss overlay、导航和菜单 surface，避免残留 Top/Overlay 阻断 primary-plane scanout。fullscreen 不再由 Shell 销毁或重建动态壁纸：`wallpaperPauseWhenFullscreen` 默认开启时交给 Wallpaper Engine 原生暂停，关闭时注入 `--no-fullscreen-pause` 保持播放；两种选择都保持 renderer/background surface 的生命周期。每个 Wallpaper Engine 输出现在携带 `--screenshot <state>/lock-wallpaper/<output>.png --screenshot-delay 5`，其 FBO 截图是实际 renderer framebuffer 尺寸而非项目 `preview.gif`；LockScreen 按输出读取该帧，缺失时持续重试并回退到配置静态图/内置高清图，静态回退铺满，真实渲染帧保持完整不裁切。启动流程将真实 session 的预启动串行放入 `start-quickshell.sh`，child session 复用同一入口，nested preview 明确禁用 live renderer 并跳过预启动，以免触碰真实桌面 state；每个输出使用带随机 token 与 `/proc` start-time 身份核验的受监督记录并在 Quickshell 同步读取后真收养，取消旧的 takeover/cleanup 双 renderer 竞态，陈旧记录不会假收养或误杀复用 PID；输出尚未公布时不再错误启动 `default` renderer。电池运行与预启动仍统一使用 idle FPS，Weather Canvas 在 fullscreen/电池时停止动画。两个会话启动器的 `auto` 改为接电/无电池 `balanced`、电池 `power-saver`，`performance` 仅显式选择。现有 `NIRI_FRAME_TELEMETRY` 在唯一 TTY DRM `render_frame` 结果处按 Smithay `PrimaryPlaneElement::Element`/`Swapchain` 记录 direct/composited frame 和百分比，不启用 overlay planes；单测锁定 1/1、50% 汇总。旧隔离 winit + mpv 验收中 Top/Overlay surface 消失的结果仍有效，但 `linux-wallpaperengine` 消失和退出后重启已不再是验收目标；winit 后端没有 DRM primary plane，因此保留 background surface 后的 direct-scanout 比例需在下一次完整 TTY 会话用现有计数复核。原 R14 目标回归、Shell、Quickshell、niri、配置与 guardrail 门禁已通过；本次 UX 修正的锁屏/壁纸/R14 目标回归与真实 QML runner 通过，TTY 实机复核完成后再恢复 verified。 |
| R15 质量门禁与拆分 | pending |  |
