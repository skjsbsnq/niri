...
| R14 Direct scanout 与背景预算 | verified | fullscreen 状态唯一来自 Quickshell foreign-toplevel 已提交的 `fullscreen + screens`，不再用窗口尺寸猜测；TopBar、Dock、Dynamic Island 按输出真实 unmap，任意输出进入 fullscreen 时同时关闭已打开的顶栏弹窗、dismiss overlay、导航和菜单 surface，避免残留 Top/Overlay 阻断 primary-plane scanout。动态/外部壁纸在 fullscreen 完全停止，退出后按 active/idle budget 恢复；电池运行与预启动统一使用 idle FPS，Weather Canvas 在 fullscreen/电池时停止动画。两个会话启动器的 `auto` 改为接电/无电池 `balanced`、电池 `power-saver`，`performance` 仅显式选择。现有 `NIRI_FRAME_TELEMETRY` 在唯一 TTY DRM `render_frame` 结果处按 Smithay `PrimaryPlaneElement::Element`/`Swapchain` 记录 direct/composited frame 和百分比，不启用 overlay planes；单测锁定 1/1、50% 汇总。隔离 winit + mpv 实测即使先打开 BatteryPopup，fullscreen 后 layer IPC 仅余被不透明窗口遮挡的 `tahoe-wallpaper` background，三项 Tahoe Top layer、BatteryPopup、dismiss Overlay 与 linux-wallpaperengine 全部消失；退出 fullscreen 后三层和 `--fps 12` 壁纸恢复，弹窗不重开。winit 后端没有 DRM primary plane，未伪造硬件 scanout 样本；新计数将在下一次完整 TTY 会话直接报告。R14 目标回归 6/6、Shell 747/747（217 subtests）、Quickshell Tahoe CTest 12/12、niri 默认并行/单线程均 316/316、niri-config 35/35（wiki 1/1）、niri-ipc 3/3（doctest 1/1）、release build/check、changed-code rustfmt、bash syntax、qmllint 解析、配置 validate、guardrails 与 diff check 通过。 |
| R15 质量门禁与拆分 | pending |  |
