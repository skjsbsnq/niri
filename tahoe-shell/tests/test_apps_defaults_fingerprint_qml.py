...
QML_TEST = Path(__file__).with_name("tst_apps_defaults_fingerprint.qml")


def test_real_qml_apps_defaults_fingerprint() -> None:
    qt6_runner = Path("/usr/lib/qt6/bin/qmltestrunner")
    runner = str(qt6_runner) if qt6_runner.is_file() else shutil.which("qmltestrunner")
    assert runner is not None, "Qt 6 qmltestrunner is required for defaults fingerprint coverage"
    env = os.environ.copy()
