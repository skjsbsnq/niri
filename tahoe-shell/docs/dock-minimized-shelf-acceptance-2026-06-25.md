...
- Dock 普通运行窗口区改为只显示非最小化窗口。
- 新增 Dock 最小化窗口 shelf，显示最小化窗口的真实缩略图，角落叠加 app icon。
...
- Dock 结构变为：固定应用、非最小化运行窗口、最小化窗口缩略栏、Downloads、Trash。
- 最小化窗口不再重复出现在普通运行窗口区。
- 每个最小化窗口在 Dock 右侧显示独立缩略图。
...
3. 最小化其中一个窗口，确认它从普通运行窗口区移除，并出现在 Downloads/Trash 左侧的缩略栏。
4. 确认缩略栏卡片显示真实窗口内容缩略图，不只是 app icon。
5. 点击缩略图，确认窗口恢复并离开缩略栏。
6. 同一应用打开多个窗口并逐个最小化，确认 shelf 中出现多个独立缩略图。
7. 最小化多个窗口直到 shelf 超出可用宽度，确认可横向滚动，Downloads 和 Trash 仍显示且可点击。
8. 右键最小化缩略图，确认打开现有 Dock 窗口菜单，并可执行“显示窗口”和“关闭窗口”。
9. 关闭一个已最小化窗口，确认对应缩略图立即消失，runtime 缩略图文件被清理。
10. 临时让 `niri msg window-thumbnail` 失败，确认对应窗口显示 fallback 卡片且 Dock 不崩溃。
