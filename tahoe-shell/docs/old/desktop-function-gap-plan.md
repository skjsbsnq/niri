...
- 控制中心已经有真实音量、亮度、Wi-Fi、蓝牙、飞行模式本地切换、MPRIS；2026-06-15 已修复 Wi-Fi/蓝牙/飞行模式点击区域互相覆盖的问题。
...
- 2026-06-15 已补：托盘右键不再弹原生白菜单，改为自绘 Tahoe QML 菜单并吃到 niri blur；FClash SNI 图标和菜单已验证可见。Steam 这类 Xwayland/legacy tray 应用如果没有发布 SNI，仍需要 `xembedsniproxy` 之类的 XEmbed-to-SNI bridge；`xwayland-satellite` 只解决 Xwayland 窗口，不等于托盘桥接。
...
- SNI 系统托盘基础闭环已可用；剩余风险是 Steam、部分输入法、云同步类应用可能仍走 legacy XEmbed，需要桥接服务或逐个应用确认。
- 没有夜间模式 / 深浅色模式开关。
- 没有截图、录屏、Quick Look。
...
- DND 能阻止 toast 干扰。
- 系统托盘至少能显示 SNI 图标或 fallback，右键 Tahoe 菜单可打开，attention 状态有可见提示；legacy XEmbed 应用有明确桥接方案。
- 输入法状态至少有可见指示和后续接入方案。
...
- `MenuPopup.qml` 已接 Tahoe 自己的电源/会话动作：Lock、Sleep、Logout、Restart、Shutdown 走真实命令，危险动作进入确认态。
...
- 没有最近搜索/最近应用。
- 没有菜单栏 app menu 接入。
- Tahoe 菜单仍没有真实 About、Settings 页面，也没有接应用级 app menu / dbus-menu。
- 没有右键上下文菜单。
...
7. `MenuPopup.qml` 后续补真实 About、Settings 页面；app menu/dbus-menu 作为独立专项。
...
6. 截图/录屏/Quick Look 入口。
7. Tahoe 菜单 About/Settings 和 app menu/dbus-menu 调研。
