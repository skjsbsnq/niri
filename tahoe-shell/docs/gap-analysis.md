...
**剩余未做**（留后续）：AirDrop / Stage Manager / 屏幕镜像圆钮（无后端，暂为禁用占位）、深色模式按钮（接 gsettings 是独立任务）。电池电量显示已在 2026-06-15 由顶栏 + `BatteryPopup.qml` 补齐。
...
当前 `tahoe-shell/components/TopBar.qml` 状态区已有：系统托盘 `Tray`、workspace 数字按钮、通知中心入口、电池入口、Spotlight 入口、时钟和控制中心入口。2026-06-15 已补 UPower 电池 popup，并把系统托盘从薄 Repeater 明确成 SNI 状态区：`IconImage` 渲染 `StatusNotifierItem.icon`、空图标字母 fallback、attention 红色描边。**仍没有**：输入法、控制中心图标组（Wi-Fi/BT 那组小图标）、夜间模式状态入口。
...
```text
✅ 控制中心已有真实音量/亮度/Wi-Fi/蓝牙/MPRIS；❌ 键盘亮度/夜间模式仍未做
...
> 当前项目状态：**niri fork 那一层（minimize/snap/glass shader/snap preview）是真东西且完成度不错；Quickshell shell 那一层原先是"macOS 的皮 + 没有功能的骨头"，现已完成控制中心、真通知系统、通知历史/DND、电源菜单、电池状态、托盘状态区补强、动画 spring 化、玻璃参数重做、Spotlight/Launchpad 搜索、snap preview 重做。菜单不再完全是假的，但 app menu/dbus-menu 仍未接；Spotlight 仍只是应用搜索，不是完整全局搜索**。Web 参考项目是一份现成的"功能清单 + 视觉参数表 + 动画曲线表"，应该继续当成蓝图逐项翻译成 QML。下一项：输入法状态 / Dock 右段 / Search provider。
