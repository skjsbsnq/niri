...
| Phase 3：整理 QML，为 compositor-owned glass 做准备 | 已完成 | QML 玻璃 material/radius 常量集中到 `components/TahoeGlass.js`；各玻璃对象暴露 `tahoeGlassMaterial` / `tahoeGlassRadius`；Spotlight 已拆出 search pill 与 results panel 语义；Launchpad 使用 `backdrop` material；blur-region item 几何不再使用 spring。 |
| Phase 4：niri 实现 `tahoe_glass_v1` | 已完成 | niri 已实现 Tahoe glass 私有协议 server、per-surface committed regions、namespace allowlist、material config、region 级 shadow/background-effect 渲染；最新提交为 `niri@1583df3`。 |
| Phase 5：Quickshell 实现 `TahoeGlass` | 已完成 | Quickshell fork 已实现 `TahoeGlass` attached object、`TahoeGlassRegion`、多 region、item-to-surface 坐标映射和协议不可用时的 `BackgroundEffect.blurRegion` fallback。 |
| Phase 6：迁移 Tahoe shell 组件 | 已完成 | Control Center、Notification Center、MenuPopup、TrayMenu、BatteryPopup、NotificationToast、Dock、TopBar、Spotlight 和 Launchpad 已迁移到 `TahoeGlass.regions`；显式 `BackgroundEffect.blurRegion` 已从 Tahoe QML 组件移除。 |
| Phase 7：删除旧 broad fallback 路径 | 已完成 | broad `^quickshell` glass/shadow 规则保持删除状态；Tahoe QML 禁止直接 `BackgroundEffect.blurRegion`；新增 `scripts/check-tahoe-glass-guardrails.sh` 并由 `arch-update.sh` 默认执行。 |
| Phase 8：验证矩阵 | 待做 | 执行完整视觉/运行时验证矩阵。 |
...
状态：已落地。共享 material/radius 词汇表在 `components/TahoeGlass.js`；现有 `BackgroundEffect.blurRegion` 仍作为 fallback，但每个可迁移的玻璃对象都在 QML item 上声明 `tahoeGlassMaterial` 和 `tahoeGlassRadius`。Spotlight 已明确区分 fullscreen input/scrim surface、search pill glass region 和 results panel glass region；Launchpad backdrop 不再按普通圆角 panel 表达。blur/glass region item 的 x/y/width/height 动画只使用 bounded `NumberAnimation` 或直接绑定，spring 保留在 opacity/scale 或非 region 子元素上。
...
状态：已落地到 niri `niri-tahoe-desktop` 分支，最新提交 `1583df3`（`Clean Tahoe glass build warnings`，基于 `1a90a35` 的 Phase 4 实现）。实现内容包括协议 XML、Wayland global 注册、`tahoe_glass_surface_v1` pending/committed region 状态、region 校验、namespace allowlist、`tahoe-glass` material 配置、layer surface region 级 shadow/background-effect 渲染和 debug log。Arch VM 中 release build 已完成；后续仍需在 Phase 5 客户端接入后验证真实 region 上报和视觉效果。
...
状态：已落地到 Quickshell fork。实现内容包括 `tahoe_glass_v1` client、`TahoeGlass` attached object、`TahoeGlassRegion` QML object、多 region、item-to-surface 坐标映射、material/radius/flags，以及协议不可用时自动回退到 `BackgroundEffect.blurRegion`。
...
状态：已落地到 Tahoe shell QML。所有 Tahoe 玻璃组件现在通过 `TahoeGlass.regions` 声明 compositor-owned glass region；原来的显式 `BackgroundEffect.blurRegion` 不再由各组件直接维护，fallback 交给 TahoeGlass client 统一处理。Spotlight 已使用 search pill + results panel 两个 region，Dock / TopBar 使用大 layer surface 内部的小 glass region，Launchpad 使用 fullscreen `backdrop` material 且禁用 shadow。
...
状态：已落地。`config/niri/tahoe-phase0.kdl` 没有 broad `namespace="^quickshell"` glass/shadow 规则，只保留 explicit Tahoe namespace rules；Tahoe QML 组件没有直接 `BackgroundEffect.blurRegion` 调用，玻璃路径统一声明为 `TahoeGlass.regions`；新增 `scripts/check-tahoe-glass-guardrails.sh`，并由 `scripts/arch-update.sh` 在部署前默认运行。

任务：

- 永久删除 broad `namespace="^quickshell"` glass/shadow 规则。（已完成，并由 guardrail 检查）
- 保留 explicit Tahoe namespace rules。（已完成，并由 guardrail 检查）
- 文档规定：
  - 新 Tahoe 玻璃组件必须使用 `TahoeGlass`。（已记录到 `tahoe-shell/docs/tahoe-glass-guardrails.md`）
  - `BackgroundEffect.blurRegion` 只用于非 Tahoe client 或协议 fallback。（已记录到 `tahoe-shell/docs/tahoe-glass-guardrails.md`）
- code review 规则：
  - 新增 `PanelWindow` 必须设置 namespace。（已记录并由 guardrail 检查）
  - 新增玻璃 UI 必须声明 material 和 radius。（已记录并由 guardrail 检查）

验收：

- 新增一个 Tahoe 面板不会因为默认 `"quickshell"` namespace 自动继承错误玻璃效果；`scripts/check-tahoe-glass-guardrails.sh` 会阻止缺少 `tahoe-*` namespace 的 `PanelWindow`、直接 `BackgroundEffect.blurRegion` 调用，以及缺少 material/radius 的 `TahoeGlassRegion`。
...
4. Phase 3：整理 QML，为 compositor-owned glass 做准备。
5. Phase 4：niri 实现 `tahoe_glass_v1`。
6. Phase 5：Quickshell 实现 `TahoeGlass` 客户端。
7. Phase 6：逐个迁移组件。
8. Phase 7：删除旧路径。
9. Phase 8：完整验证。
