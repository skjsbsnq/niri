...
`arch-update.sh` also maintains a patched `xwayland-satellite` under the legacy path `~/.local/lib/niri/xwayland-satellite-minimize` by default. This is the X11 fallback needed by apps such as Linux WeChat whose visible window still goes through xwayland-satellite. The combined patch bridges EWMH maximize requests, forwards minimize requests, refreshes stale Wayland clipboard offers, maps X11 `UTF8_STRING` to Wayland text MIME types, and uses `ext-data-control-v1` when no input serial exists. Every satellite rebuild runs the full upstream unit and integration suite before installing the binary. See `docs/old/xwayland-satellite-clipboard.md` before changing the patch or clipboard bridge behavior.

The Tahoe session defaults `ELECTRON_OZONE_PLATFORM_HINT=auto`; Electron 39 and newer ignore this legacy environment variable, so backend-specific workarounds use explicit per-app flags. Linux QQ currently needs `--ozone-platform=x11` in `~/.config/qq-flags.conf`: native Wayland clipboard writes can be rejected after QQ loses focus while its asynchronous chat-copy operation still reports success. XWayland text can be slightly softer at the configured `1.25` scale, but reliable cross-application clipboard ownership takes precedence. Remove that per-app flag only after a real `QQ -> wl-paste -> Codex -> cliphist` test passes on native Wayland.

The niri Tahoe config intentionally points at `~/.local/lib/niri/xwayland-satellite-minimize-glamor`, a wrapper generated by `arch-update.sh`. The wrapper preserves the combined Tahoe compatibility patch and forces `-glamor gl`, because on the hybrid AMD + NVIDIA Tahoe setup Xwayland GLX clients otherwise fall back to Mesa llvmpipe. The config and Tahoe session launchers also set `__GLX_VENDOR_LIBRARY_NAME=nvidia` so GLVND selects the NVIDIA GLX vendor. See `docs/old/xwayland-satellite-nvidia-glx.md` before changing this path or environment variable.
...
If upstream has merged equivalent maximize, minimize, and clipboard support and the local patch no longer applies, the script can build without the local patch after detecting all three capability groups. Otherwise it stops with a clear patch-update error. Set `BUILD_XWAYLAND_SATELLITE=false` to skip this stage temporarily.
