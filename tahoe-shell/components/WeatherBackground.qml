...
//     与 Tahoe 深/浅色玻璃语言一致。
//   - 全部粒子由单个 16ms Timer 统一驱动、在单个 Canvas 里绘制（参考同结构），不拆多 Canvas。
//
// 安全约束（路线图 §6.2 / 风险 1、6）：
//   - 不喂任何几何给 TahoeGlassRegion（本组件无玻璃区域，纯 Canvas 绘制），故无弹簧崩风险。
//   - 视差用 Behavior on pointerX/Y（NumberAnimation，非几何属性），照参考。
//   - Timer.running 守门于 `visible && animate`，切走/关闭即停（风险 6 对策，DoD 要求）。
Item {
    id: root
    ...
    function basePalette() {
        ...
        case "overcast_day":   return sky("#9aadb9", "#c8d2d8", "#eef3f5", "#f7fafb", "#ecebea", "#dddcd9", "#cbc9c6", "#eaf0f5", "#dce7ef")
        case "overcast_night": return sky("#53657a", "#8a9bad", "#c1ccd4", "#e7edf1", "#dde2eb", "#c3c8d2", "#aeb4c0", "#dae6f4", "#c2d2e5")
        case "rain_day":       return sky("#a5b7d2", "#ced8e4", "#edf2f7", "#f6f9fd", "#ecebea", "#dddcd9", "#cbc9c6", "#d8ecff", "#cce0f6")
        case "rain_night":     return sky("#4a617d", "#879cb7", "#bcc9d9", "#e3ebf4", "#dde2eb", "#c3c8d2", "#aeb4c0", "#cee5ff", "#aac6e7")
    ...
    // 落叶由主 Timer 驱动（与其它粒子统一），用累积时间触发新叶。
    ...
    // ============================================================
    // Section 16: 驱动 Timer（16ms，守门 visible && animate）
    // ============================================================
    ...
    Timer {
        ...
        onTriggered: {
            var now = Date.now()
            var dt = lastTickMs > 0 ? Math.min(0.05, (now - lastTickMs) / 1000.0) : interval / 1000.0
