...
mod tests {
    ...
    fn window(id: u64, is_minimized: bool) -> Window {
        Window {
            id,
            ...
            layout: WindowLayout {
                ...
                tile_size: (100., 200.),
                window_size: (100, 200),
                tile_pos_in_workspace_view: Some((0., 0.)),
    ...
    #[test]
    fn windows_state_tracks_minimized_changes() {
        let mut state = WindowsState::default();
        assert!(state
            .apply(Event::WindowOpenedOrChanged {
                window: window(1, false),
            })
            .is_none());
        assert!(!state.windows.get(&1).unwrap().is_minimized);
        ...
        let Event::WindowsChanged { windows } = &events[0] else {
            panic!("expected WindowsChanged");
        };
